  ...
  return (
    <aside className="admin-sidebar flex w-64 shrink-0 flex-col border-r border-landing bg-landing-surface backdrop-blur-xl">
      ...
      <div className="border-t border-landing p-4">
        <Link
          href="/"
          className="admin-segment-btn block rounded-xl border border-landing px-3 py-2 text-center text-xs"
        >
          ← Back to site
        </Link>
      </div>
...
function DashboardIcon({ active }: { active: boolean }) {
  return (
    <svg className={`h-5 w-5 shrink-0 ${active ? "" : "text-landing-muted"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
